...
const mapReleases = (request, expected) => {
    return request.assets.find( asset => {
        let assetParsed = path.parse(asset.name).name
            .replace(/\.[^/.]+$/, "")
            .split('_')
        let assetRuntime = {
            arch: assetParsed.pop(),
            platform: assetParsed.pop()
        }
        return JSON.stringify(assetRuntime) == JSON.stringify(expected)
    })
}
